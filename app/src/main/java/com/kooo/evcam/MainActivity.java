...
public class MainActivity extends AppCompatActivity {
                ...
                if (cameraManager != null && !cameraManager.isRecording()) {
                    ...
                    new android.os.Handler(android.os.Looper.getMainLooper()).postDelayed(() -> {
                        if (cameraManager != null && cameraManager.hasConnectedCameras() && !cameraManager.isRecording()) {
                            ...
                            Toast.makeText(this, "Camera not ready, please retry", Toast.LENGTH_SHORT).show();
                        }
                    }, 2000);
                }
            }, 1000);
    ...
    private void toggleSupervisionMode() {
        ...
        // 显示提示
        String message = newEnabled ? "Supervision mode enabled" : "Supervision mode disabled";
    ...
    private void initCamera() {
        ...
        if (existingManager != null) {
                    ...
                    runOnUiThread(() -> {
                        if (status.contains("ERROR_CAMERA_IN_USE") || status.contains("DISCONNECTED")) {
                            Toast.makeText(MainActivity.this,
                                "Camera " + cameraId + " in use, auto-reconnecting...",
                                Toast.LENGTH_SHORT).show();
                        } else if (status.contains("max reconnect attempts")) {
                            Toast.makeText(MainActivity.this,
                                "Camera " + cameraId + " reconnection failed, please restart app",
            ...
            cameraManager.setCodecFallbackCallback(() -> {
                runOnUiThread(() -> {
                    Toast.makeText(this,
                        "Recording error, fell back to MediaCodec mode. If frequent, change mode manually",
                        Toast.LENGTH_LONG).show();
                });
            });
            ...
            cameraManager.setRecordingStatusCallback((activeCameras, failedCameras) -> {
                ...
                if (activeCameras.isEmpty()) {
                    ...
                    runOnUiThread(() -> {
                        ...
                        isPreparingRecording = false;
                        hidePreparingIndicator();
                        Toast.makeText(this, "Recording start failed, please retry", Toast.LENGTH_SHORT).show();
                    });
                }
            });
    ...
    private void updateDebugInfo() {
        ...
        // 录制状态
        sb.append("\n\n");
        sb.append("Record: ").append(isRecording ? "● REC" : "○ Stopped");
        if (isRecording) {
            sb.append("  Mode: ").append(appConfig.getRecordingMode());
        }
        ...
        sb.append("\n");
        sb.append("Memory: ").append(usedMB).append("/").append(totalMB).append(" MB");

        // 车型
        sb.append("\n");
        sb.append("Car: ").append(appConfig.getCarModel());
        sb.append("  Cameras: ").append(appConfig.getCameraCount());
    ...
    private void checkResumeRecordingAfterRecreate() {
        ...
        new android.os.Handler(android.os.Looper.getMainLooper()).postDelayed(() -> {
            ...
            if (cameraManager == null || !cameraManager.hasConnectedCameras()) {
                ...
                Toast.makeText(this, "Camera not ready, resume recording failed", Toast.LENGTH_SHORT).show();
                shouldResumeRecordingAfterRecreate = false;
                savedRecordingStartTime = 0;
                savedSegmentCount = 1;
                return;
            ...
            startRecording();
            Toast.makeText(this, "Auto-resumed recording", Toast.LENGTH_SHORT).show();
    ...
    private void checkAutoStartRecording() {
        ...
        new android.os.Handler(android.os.Looper.getMainLooper()).postDelayed(() -> {
            ...
            if (cameraManager == null || !cameraManager.hasConnectedCameras()) {
                ...
                Toast.makeText(this, "Camera not ready, auto recording failed", Toast.LENGTH_SHORT).show();
                return;
            }
            
            AppLog.d(TAG, "自动开始录制...");
            startRecording();
            Toast.makeText(this, "Auto-started recording", Toast.LENGTH_SHORT).show();
        }, 2000);  // 延迟2秒
    }
    ...
    private void onScreenOff() {
        ...
        if (isRecording) {
            ...
            screenOffStopRunnable = () -> {
                ...
                stopRecording();
                runOnUiThread(() -> {
                    Toast.makeText(MainActivity.this, "Screen off 10s, auto-stopped recording", Toast.LENGTH_SHORT).show();
                });
            };
    ...
    private void scheduleBackgroundTask() {
        ...
        screenOffBackgroundRunnable = () -> {
            ...
            runOnUiThread(() -> {
                Toast.makeText(MainActivity.this, "Screen off 15s, moved to background", Toast.LENGTH_SHORT).show();
            });
        };
    ...
    private void onScreenOn() {
        ...
        screenOnStartRunnable = () -> {
                    ...
                    try {
                        ...
                        screenStateHandler.postDelayed(() -> {
                            if (!isScreenOff && !isRecording && cameraManager.hasConnectedCameras()) {
                                ...
                                startRecording();
                                Toast.makeText(MainActivity.this, "Auto-resumed recording", Toast.LENGTH_SHORT).show();
                            }
                        }, 2000);
                    } catch (Exception e) {
            ...
            startRecording();
            runOnUiThread(() -> {
                Toast.makeText(MainActivity.this, "Screen on 10s, auto-resumed recording", Toast.LENGTH_SHORT).show();
            });
        };
    ...
    private void startRecording() {
        if (cameraManager != null && !cameraManager.isRecording()) {
            ...
            if (success) {
                ...
                // 启动前台服务保护（防止后台录制被中断）
                CameraForegroundService.start(this, "Recording video", "Recording in progress, tap to return");
                ...
                if (isFallback && !AppConfig.isSdFallbackShownThisSession()) {
                    ...
                    Toast.makeText(this, "USB drive not detected, fallback to internal storage", Toast.LENGTH_LONG).show();
                    AppLog.w(TAG, "U盘回退：用户选择U盘但不可用，使用内部存储");
                } else {
                    // 显示录制的摄像头数量
                    int cameraCount = enabledCameras.size();
                    String cameraText = cameraCount == appConfig.getCameraCount() ? "All" : cameraCount + "";
                    Toast.makeText(this, "Recording " + cameraText + " camera(s)", Toast.LENGTH_SHORT).show();
                }
    ...
    public void startDingTalkService() {
        if (!dingTalkConfig.isConfigured()) {
            Toast.makeText(this, "Please configure DingTalk settings first", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        DingTalkStreamManager.ConnectionCallback connectionCallback = new DingTalkStreamManager.ConnectionCallback() {
            ...
            public void onConnected() {
                runOnUiThread(() -> {
                    ...
                    Toast.makeText(MainActivity.this, "DingTalk remote started", Toast.LENGTH_SHORT).show();
    ...
    public void stopDingTalkService() {
        if (dingTalkStreamManager != null) {
            ...
            Toast.makeText(this, "Remote view service stopped", Toast.LENGTH_SHORT).show();
            // 通知 RemoteViewFragment 更新 UI
            updateRemoteViewFragmentUI();
        }
    }
        ...
        try {
            ...
            if (isRecording) {
                ...
                if (recordingStartTime > 0) {
                    ...
                    long minutes = totalSeconds / 60;
                    long seconds = totalSeconds % 60;
                    sb.append("⏱️ Duration: ").append(String.format("%02d:%02d", minutes, seconds));
                    sb.append(" / Segment ").append(currentSegmentCount).append("\n");
                }
            } else {
                sb.append("🎬 Recording: Not recording\n");
            }
            ...
            if (cameraManager != null) {
                ...
                sb.append("📷 Cameras: ").append(connectedCount).append("/").append(totalCount).append(" connected\n");
            } else {
                sb.append("📷 Cameras: Not initialized\n");
            }
            ...
            try {
                ...
                if (storageDir != null && storageDir.exists()) {
                    ...
                    sb.append("💾 Storage: ").append(useExternal ? "USB" : "Internal");
                    sb.append(" (").append(availableStr).append(" free)\n");
                }
            } catch (Exception e) {
                // 忽略存储获取错误
            }
            ...
            sb.append("📱 App: ").append(isInBackground ? "Background" : "Foreground").append("\n");

            // 分隔线
            sb.append("━━━━━━━━━━━━━━\n");

            // 设置摘要
            sb.append("⚙️ Settings:\n");

            // 自动录制
            sb.append("• Auto Record: ").append(appConfig.isAutoStartRecording() ? "On" : "Off");
            if (appConfig.isAutoStartRecording() && appConfig.isScreenOffRecordingEnabled()) {
                sb.append("+ScreenOff");
            }
            sb.append("\n");
            ...
            if (heartbeatManager != null) {
                ...
                if (hbConfig.isEnabled()) {
                    ...
                    sb.append("• Heartbeat Push: Off\n");
                }
            }

            // 分段时长
            int segmentMin = appConfig.getSegmentDurationMinutes();
            sb.append("• Segment Duration: ").append(segmentMin).append("min\n");

            // 车型
            sb.append("• Car Model: ").append(appConfig.getCarModel());
            
        } catch (Exception e) {
            AppLog.e(TAG, "构建状态信息失败", e);
            sb.append("Failed to get status info: ").append(e.getMessage());
        }
    ...
    private String handleStartRecordingCommand() {
        ...
        // 如果已经在录制，返回提示
        if (isRecording) {
            return "⚠️ Already recording, no need to start again";
        }
        ...
        return "▶️ Starting recording...\n\nSend \"Status\" to check recording status\nSend \"Stop Recording\" to stop recording";
    }
    ...
    private String handleStopRecordingCommand() {
        ...
        if (recordingStartTime > 0) {
            ...
            long minutes = totalSeconds / 60;
            long seconds = totalSeconds % 60;
            durationInfo = String.format(", recorded %02d:%02d", minutes, seconds);
        }
        ...
        return "⏹️ Recording stopped" + durationInfo + "\nApp will go to background";
    }
    ...
    private String handleForegroundCommand() {
        ...
        return "📱 App switched to foreground";
    }
    ...
    private String handleBackgroundCommand() {
        ...
        return "📴 App switched to background";
    }
    ...
    private String buildHeartbeatStatusJson() {
        ...
        try {
            ...
            sb.append("\"availableSpaceText\":\"Unknown\",");
        }
    ...
    public void showImageAdjustFloatingWindow() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && !android.provider.Settings.canDrawOverlays(this)) {
            Toast.makeText(this, "Overlay permission required to open adjustment window", Toast.LENGTH_SHORT).show();
    ...
    public void showCameraPreviewFloating(String cameraPosition) {
        // 检查悬浮窗权限
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && !android.provider.Settings.canDrawOverlays(this)) {
            Toast.makeText(this, "Overlay permission required to show preview", Toast.LENGTH_SHORT).show();
            Intent intent = new Intent(android.provider.Settings.ACTION_MANAGE_OVERLAY_PERMISSION,
                    android.net.Uri.parse("package:" + getPackageName()));
            startActivity(intent);
            return;
        ...
        Toast.makeText(this, "Camera preview floating window not yet implemented", Toast.LENGTH_SHORT).show();
    }
